"""
PHASE 5 — Controlled Backbone Fine-Tuning Script
Key improvements over Phase 4:
  1. Unfreeze last 2 ViT-Base blocks (10, 11) — controlled backbone adaptation
  2. Differential LR: backbone 5e-6, head 2e-4 (40x ratio)
  3. Loss rebalance: Focal γ=2.0 (weight=0.3) + Dice (weight=0.7)
  4. Reduced augmentation intensity for stable backbone gradients
  5. Resume from Phase 4 best checkpoint (IoU=0.5150)
  6. 30 epochs with patience 10
  7. Enhanced overfit monitoring: gap ≤0.05, consecutive degradation check
...
def get_train_transform(h, w):
    """Phase 5 train augmentations — REDUCED intensity for backbone fine-tuning stability."""
    return A.Compose([
        A.Resize(h, w),
        A.HorizontalFlip(p=0.5),
        A.VerticalFlip(p=0.1),
        # Narrower multi-scale: 0.9x–1.1x (was 0.8x–1.2x in Phase 4)
        ...
        # RandomShadow REMOVED — expert caution: noisy gradients during backbone unfreeze
...
# ============================================================================
# Losses: Focal (gamma=2.0, weight=0.3) + Dice (weight=0.7)
# ============================================================================
...
class CombinedLoss(nn.Module):
    def __init__(self, alpha=None, focal_weight=0.3, dice_weight=0.7, gamma=2.0, num_classes=10):
        super().__init__()
...
# ============================================================================
# UPerNet Segmentation Head (identical to Phase 3/4)
# ============================================================================
...
def save_plots(history, output_dir):
    ...
    for ax, key, title in [
        ...
        if key == 'iou':
            ...
            # Also mark the overfit danger zone
            ax.axhline(y=0.05, color='orange', linestyle=':', alpha=0.3, label='Gap threshold')
        ax.set_title(title, fontsize=12, fontweight='bold')
        ax.set_xlabel('Epoch'); ax.set_ylabel(title)
        ax.legend(); ax.grid(True, alpha=0.3)

    plt.suptitle('Phase 5 — Controlled Backbone Fine-Tuning', fontsize=14, fontweight='bold')
    plt.tight_layout()
    ...
    # LR schedule (both backbone and head)
    if 'lr_head' in history:
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 4))
        ax1.plot(history['lr_backbone'], linewidth=2, color='purple', label='Backbone LR')
        ax1.set_title('Backbone LR (blocks 10-11)', fontweight='bold')
        ax1.set_xlabel('Epoch'); ax1.set_ylabel('LR'); ax1.grid(True, alpha=0.3); ax1.legend()
        ax2.plot(history['lr_head'], linewidth=2, color='green', label='Head LR')
        ax2.set_title('Head LR (UPerNet)', fontweight='bold')
        ax2.set_xlabel('Epoch'); ax2.set_ylabel('LR'); ax2.grid(True, alpha=0.3); ax2.legend()
        plt.suptitle('Differential Learning Rates (Phase 5)', fontweight='bold')
        plt.tight_layout()
    ...
    if 'gap' in history:
        ...
        plt.title('Train-Val IoU Gap (Overfit Monitor)', fontweight='bold')
...
def save_metrics(history, output_dir, config):
    ...
    with open(filepath, 'w') as f:
        f.write("PHASE 5 — CONTROLLED BACKBONE FINE-TUNING RESULTS\n" + "="*80 + "\n\n")
        ...
        hdr = f"{'Ep':<5}{'TrLoss':<10}{'VaLoss':<10}{'TrIoU':<10}{'VaIoU':<10}{'TrDice':<10}{'VaDice':<10}{'TrAcc':<10}{'VaAcc':<10}{'Gap':<10}{'LR_bb':<12}{'LR_head':<12}\n"
        f.write(hdr + "-"*140 + "\n")
...
def main():
    ...
    w = 46 * 14  # 644
    h = 26 * 14  # 364
    lr_backbone = 5e-6         # Very slow for unfrozen blocks
    lr_head = 2e-4             # Normal for head
    n_epochs = 30
    warmup_epochs = 3
    patience = 10
    use_amp = True
    ...
    config = {
        'phase': 'Phase 5 — Controlled Backbone Fine-Tuning',
        'backbone': 'dinov2_vitb14_reg (ViT-Base) — blocks 10-11 UNFROZEN',
        'seg_head': 'UPerNet (PPM + multi-scale FPN)',
        'loss': 'Focal (gamma=2.0, weight=0.3) + Dice (weight=0.7)',
        ...
        'resume_from': 'Phase 4 best checkpoint (IoU=0.5150)',
        'tta': 'HorizontalFlip average at eval',
        'backbone_unfreeze': 'Blocks 10-11 only (last 2 of 12)',
    }
    ...
    output_dir = os.path.join(project_root, 'TRAINING AND PROGRESS', 'PHASE_5_CONTROLLED')
    ...
    for i, block in enumerate(backbone.blocks):
        if i >= 10:  # blocks 10 and 11
            for param in block.parameters():
                param.requires_grad = True
                unfrozen_count += param.numel()

    # Keep patch_embed, pos_embed, cls_token, and norm frozen
    # (they're already frozen from the blanket freeze above)
    ...
    print(f"Backbone: {backbone_trainable:,} / {backbone_total:,} params unfrozen ({backbone_trainable/backbone_total*100:.1f}%)")
    print(f"  Unfrozen blocks: 10, 11 (last 2 of {len(backbone.blocks)})")

    # Set backbone to train mode for unfrozen blocks (but eval for frozen)
    backbone.train()

    # Get embedding dim
    backbone.eval()  # Temporarily eval for inference
    ...
    backbone.train()  # Back to train mode
    ...
    # ── Resume from Phase 4 ──
    p4_checkpoint = os.path.join(project_root, 'TRAINING AND PROGRESS', 'PHASE_4_MASTERY', 'best_model.pth')
    if os.path.exists(p4_checkpoint):
        print(f"Loading Phase 4 checkpoint: {p4_checkpoint}")
        ckpt = torch.load(p4_checkpoint, map_location=device, weights_only=False)
        model.load_state_dict(ckpt['model_state_dict'])
        print(f"  Loaded from Epoch {ckpt.get('epoch', '?')}, Val IoU: {ckpt.get('val_iou', '?')}")
    else:
        # Try Phase 3 if Phase 4 not available
        ...
        if os.path.exists(p3_checkpoint):
            print(f"Phase 4 checkpoint not found. Loading Phase 3: {p3_checkpoint}")
    ...
    print(f"Total trainable: {total_trainable:,} (backbone {backbone_trainable:,} + head {head_params:,})")

    # ── Loss ──
    class_weights = compute_class_weights(train_loader, n_classes, device)
    loss_fn = CombinedLoss(alpha=class_weights, focal_weight=0.3, dice_weight=0.7, gamma=2.0, num_classes=n_classes)
    ...
    optimizer = optim.AdamW([
        {'params': backbone_params, 'lr': lr_backbone},   # Backbone: very slow
        {'params': head_params_list, 'lr': lr_head},       # Head: normal
    ], weight_decay=1e-4)

    # Warmup + cosine (applies to both param groups)
    def lr_lambda(epoch):
    ...
    consecutive_val_drops = 0  # Safety: track consecutive val IoU drops
    prev_val_iou = 0.0
    start_time = time.time()

    # ── Training ──
    print(f"\nPhase 5 Controlled Training: {n_epochs} epochs, {w}x{h}, ViT-Base (blocks 10-11 unfrozen)")
    print(f"  Focal(gamma=2.0, w=0.3) + Dice(w=0.7) | Scale 0.9x-1.1x")
    print(f"  Backbone LR: {lr_backbone} | Head LR: {lr_head}")
    print(f"  Resume from Phase 4 (IoU=0.5150)")
    print("="*80)

    for epoch in tqdm(range(n_epochs), desc="Training", unit="epoch"):
        model.train()
        backbone.train()  # Enable dropout/etc in unfrozen blocks
        train_losses = []
        optimizer.zero_grad()
        ...
        for step, (imgs, labels) in enumerate(tqdm(train_loader, desc=f"Ep {epoch+1}", leave=False)):
            ...
            with torch.amp.autocast('cuda', enabled=use_amp):
                # Backbone forward — gradients flow through unfrozen blocks
                tokens = backbone.forward_features(imgs)["x_norm_patchtokens"]
                logits = model(tokens)
                out = F.interpolate(logits, size=imgs.shape[2:], mode="bilinear", align_corners=False)
                labels = labels.squeeze(1).long()
        ...
        for k, v in [('train_loss',tr_loss),('val_loss',va_loss),('train_iou',tr_iou),('val_iou',va_iou),
                      ('train_dice',tr_dice),('val_dice',va_dice),('train_pixel_acc',tr_acc),('val_pixel_acc',va_acc),
        ...
        if va_iou > best_iou:
            ...
            torch.save(ckpt_data, os.path.join(models_dir, f'phase5_best_model_iou{va_iou:.4f}.pth'))
        ...
        # Check consecutive val drops (overfit signal)
        if epoch > 0 and va_iou < prev_val_iou:
            consecutive_val_drops += 1
        else:
            consecutive_val_drops = 0

        if consecutive_val_drops >= 3:
            overfit_warn = " ⛔ 3 consecutive val drops!"

        prev_val_iou = va_iou

        tqdm.write(f"  Ep {epoch+1:>2} | loss: {tr_loss:.3f}/{va_loss:.3f} | IoU: {tr_iou:.3f}/{va_iou:.3f} | "
                   f"gap: {gap:.3f}{gap_warn} | best: {best_iou:.3f} | "
                   f"lr_bb: {cur_lr_bb:.1e} lr_hd: {cur_lr_hd:.1e}{overfit_warn}")

        # Safety stop: 3+ consecutive val drops = likely overfitting backbone
        if consecutive_val_drops >= 3 and gap > 0.05:
            print(f"\n  ⛔ SAFETY STOP: Overfitting detected! 3 consecutive val drops + gap={gap:.3f} > 0.05")
    ...
    # Also load the fine-tuned backbone blocks
    if 'backbone_state_dict' in best_ckpt:
        backbone_state = backbone.state_dict()
        backbone_state.update(best_ckpt['backbone_state_dict'])
    ...
    config['backbone_trainable_params'] = backbone_trainable
    config['total_trainable_params'] = total_trainable
    ...
    print(f"\n{'='*80}")
    print("PHASE 5 CONTROLLED FINE-TUNING COMPLETE")
    ...
    print(f"  Phase 4 mastery:  0.5150 (TTA: 0.5169)")
    print(f"  Phase 5 controlled: {best_iou:.4f} (TTA: {tta_iou:.4f})")
    print(f"  Total improvement: {((best_iou-0.2971)/0.2971*100):.1f}% over baseline")
    print(f"  Backbone params unfrozen: {backbone_trainable:,}")
